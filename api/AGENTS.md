...
- `APP_USER` - Required system user whose home directory the API manages (`"nick"` or `"limited_user"`). Controls all user-specific paths via `src/config/appUser.ts`
- `STAGING_DIR` - Required directory for staging files before `sudo mv` to system directories. Auto-created on startup. Must match sudoers rules permitting `sudo mv <STAGING_DIR>/* /etc/systemd/system/` and `sudo mv <STAGING_DIR>/* /etc/nginx/sites-available/`
