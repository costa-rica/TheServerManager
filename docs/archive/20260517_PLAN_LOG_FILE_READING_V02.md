...
# Plan V02: Make TSM read date-suffixed log files

Supersedes [20260517_PLAN_LOG_FILE_READING.md](20260517_PLAN_LOG_FILE_READING.md). The V01 regex did not match the real overflow filenames produced by `winston-daily-rotate-file`, and the behavior of `/services/logs/:name` (single segment vs. concatenated day) was left undefined. This version fixes both.
...
V01 of this plan assumed both Node and Python would produce within-day overflow as `{name}-YYYY-MM-DD.N.log`. In practice the recommended libraries do not produce that exact form:
...
TSM must accept all three patterns and not break on legacy `{NAME}.log`.
...
This is documented explicitly so future readers know the trade-off is intentional.
...
3. If no matches, return `{ success: false, error: ... }` whose message lists both lookups that were attempted (the legacy filename and the date-suffixed glob).
4. Otherwise `fs.stat` each candidate, pick the one with the newest `mtime`, read it, return it.

#### Regex

Escape `name` before building the pattern (service names may contain regex metacharacters, even though current ones don't).
...
`GoLightly04API-2026-05-17.log.1` sorts *after* `GoLightly04API-2026-05-17.log` (good for Node), but `GoLightly04API-2026-05-17.2026-05-17_12-56-30_269989.log` sorts *before* `GoLightly04API-2026-05-17.log` because `.2` < `.l` in ASCII. Name-sort would pick the wrong file for Loguru output. `mtime` is reliable across all three patterns.
...
- `returns_newest_loguru_overflow_by_mtime` — `{name}-2026-05-17.log` + `{name}-2026-05-17.2026-05-17_12-56-30_269989.log` (newer mtime) → returns the timestamped file.
...
- [docs/references/log-file-instructions/LOGGING_NODE_JS_V08.md](references/log-file-instructions/LOGGING_NODE_JS_V08.md) — already updated to reflect `.log.N` overflow
- [docs/references/log-file-instructions/LOGGING_PYTHON_V07.md](references/log-file-instructions/LOGGING_PYTHON_V07.md) — already updated to single-sink + Loguru rename
...
4. **Loguru overflow simulation** — in a scratch dir, `touch` `{name}-2026-05-17.log`, then later `{name}-2026-05-17.2026-05-17_12-56-30_269989.log`. Expect the timestamped file to win on mtime.
5. **Missing both** — directory exists but contains no matching file. Expect `{success: false}` with a message that names both lookups.
6. **End-to-end** — run the API in `testing` or `production` `NODE_ENV` against a machine whose `servicesArray` includes both a legacy service and a date-suffixed service; hit `GET /services/logs/:name` for each from the web UI and confirm both render.
